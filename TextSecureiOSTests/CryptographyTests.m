...
-(void) testAES256RNEncryption {
  ...
  NSData *decryptedData  = [RNDecryptor decryptData:encryptedData
                                       withPassword:password
                                              error:&error];
  STAssertEqualObjects(data, decryptedData, @"encrypted data should equal decrypted data");
}

-(void)  testSHA1 {
  NSLog(@"SHA1 %@",[Cryptography computeSHA1DigestForString:@""]);
  STAssertEqualObjects([Cryptography computeSHA1DigestForString:@""],@"da39a3ee5e6b4b0d3255bfef95601890afd80709",@"Sha1 of the empty string wrong.");
}
...
-(void)testComputeMAC {
  for (int i=0;i<20;i++) {
    ...
    STAssertEqualObjects(hashValue, desiredValue, @"HMAC value not equal to that expected");
  }
}
